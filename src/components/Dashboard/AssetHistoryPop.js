  ...
  return (
    <Modal open={open} onClose={onClose}>
      ...
      >
        ...
        <Typography variant="h4" sx={{ mb: 4, textAlign: 'center', color: '#000' , fontFamily: 'Racing Sans One' }}>
        ...
        {history.length === 0 ? (
          ...
          <Box sx={{ position: 'relative', minHeight: history.length * (boxHeight + verticalGap) + 100 }}>
            {/* Boxes */}
  ...
  {entry.assignedDate ? new Date(entry.assignedDate).toLocaleDateString('en-GB') : 'N/A'}
            ...
            >
              ...
              {history.map((entry, index) => {
                if (index === 0) {
                  const isLeft = index % 2 === 0;
                  const x = centerX + (isLeft ? -boxWidth - 40 : 40) + boxWidth / 2;
                  const y = index * (boxHeight + verticalGap);
                  return (
                    <line
                      key="start"
                ...
                const x1 = centerX + (prevIsLeft ? -boxWidth - 40 : 40) + boxWidth / 2;
                const y1 = (index - 1) * (boxHeight + verticalGap) + boxHeight;

                const x2 = centerX + (currIsLeft ? -boxWidth - 40 : 40) + boxWidth / 2;
